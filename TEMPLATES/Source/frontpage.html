<html>
	<head>
		...
		$(document).ready(function() {{/literal}
			{include file="player.js" player_id="1" playlist=$daily_player_json}
			{include file="player.js" player_id="2" playlist=$weekly_player_json}
			{include file="player.js" player_id="3" playlist=$monthly_player_json}
                        {literal}
		});{/literal}//]]></script>
			...
			<table>
				...
				<tbody>
					...
					<tr bgcolor="{cycle values="#eeeeee,#dddddd"}">
				    	<td>{$position}</td>
				    	<td>"<a href="{$track.strTrackUrl}">{$track.strTrackName}</a>" by "<a href="{$track.objArtist.strArtistUrl}">{$track.objArtist.strArtistName}</a>"</td>
